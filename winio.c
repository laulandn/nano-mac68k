/* $Id: winio.c,v 1.83 2001/03/22 23:12:22 astyanax Exp $ */
/**************************************************************************
 *   winio.c                                                              *
 *                                                                        *
	       ...
	       int start_x)
{
    ...
    inputbuf = (char *)nmalloc(strlen(def) + 1);
    inputbuf[0] = 0;
	...
	for (j = 0; j <= slen - 1; j++) {
		...
		answer = (char *)mallocstrcpy(answer, inputbuf);
		free(inputbuf);
		return s[j].val;
	    }
	}
	...
	default:
	    ...
	    inputbuf = (char *)nrealloc(inputbuf, inputlen + 2);
...
inline int get_page_from_virtual(int virtual1)
{
    int page = 2;
...
void update_line(filestruct * fileptr, int index)
{
    ...
    len = strlen(realdata);
    fileptr->data = (char *)nmalloc(xpt(fileptr, len) + 1);
...
void do_replace_highlight(int highlight_flag, char *word)
{
    ...
    highlight_word = (char *)mallocstrcpy(highlight_word, &current->data[current_x]);
