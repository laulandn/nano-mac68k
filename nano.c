/* $Id: nano.c,v 1.143 2001/03/18 18:55:06 astyanax Exp $ */
/**************************************************************************
 *   nano.c                                                               *
 *                                                                        *
...
void die(char *msg, ...)
{
	...
	char *buf = (char *)nmalloc(strlen(filename) + 6);
	strcpy(buf, filename);
	strcat(buf, ".save");
	i = write_file(buf, 1);
	name = buf;
...
filestruct *copy_node(filestruct * src)
{
    ...
    dst->data = (char *)nmalloc(strlen(src->data) + 1);
...
void align(char **strp)
{
    ...
    *strp = (char *)nrealloc(*strp, strlen(*strp) + 1);
}
...
filestruct *make_new_node(filestruct * prevnode)
{
    ...
    newnode = (filestruct *)nmalloc(sizeof(filestruct));
    newnode->data = NULL;
...
void do_char(char ch)
{
    ...
    current->data = (char *)nrealloc(current->data, strlen(current->data) + 2);
...
int do_enter(filestruct * inptr)
{
	...
	if (spc) {
	    ...
	    newnode->data = (char *)nmalloc(strlen(tmp) + extra + 1);
	    strncpy(newnode->data, current->data, extra);
	    strcpy(&newnode->data[extra], tmp);
	}
    } else {
	newnode->data = (char *)nmalloc(strlen(tmp) + 1);
...
void do_wrap(filestruct * inptr, char input_char)
{
    ...
    temp = (filestruct *)nmalloc(sizeof(filestruct));
	...
	temp->data = (char *)nmalloc(strlen(&inptr->data[current_word_start]) + 1);
	strcpy(temp->data, &inptr->data[current_word_start]);
	inptr->data = (char *)nrealloc(inptr->data, last_word_end + 2);
	...
	if (current_x < current_word_start) {
	    temp->data =
		(char *)nmalloc(strlen(&inptr->data[current_word_start]) + 1);
	    strcpy(temp->data, &inptr->data[current_word_start]);
	    ...
	    inptr->data = (char *)nrealloc(inptr->data, i + 2);
	    inptr->data[i + 1] = 0;
	}
	...
	char *p = (char *)nmalloc((strlen(temp->data) + strlen(inptr->next->data) + 2) 
			* sizeof(char));
	...
	if (ISSET(AUTOINDENT)) {
		...
		t = (char *)nmalloc(strlen(temp->data) + extra + 1);
...
int do_backspace(void)
{
	...
	previous->data = (char *)nrealloc(previous->data,
...
int do_int_spell_fix(char *word)
{
    ...
    search_init_globals();
    save_search = (char *)mallocstrcpy(save_search, last_search);
    save_replace = (char *)mallocstrcpy(save_replace, last_replace);

    /* set search/replace strings to mis-spelt word */
    prevanswer = (char *)mallocstrcpy(prevanswer, word);
    last_search = (char *)mallocstrcpy(last_search, word);
    last_replace = (char *)mallocstrcpy(last_replace, word);
    ...
    last_search = (char *)mallocstrcpy(last_search, save_search);
    last_replace = (char *)mallocstrcpy(last_replace, save_replace);
...
int do_int_speller(char *tempfile_name)
{
    ...
    read_buff = (char *)nmalloc( pipe_buff_size + 1 );
...
void handle_sigwinch(int s)
{
    ...
    hblank = (char *)nrealloc(hblank, COLS + 1);
    memset(hblank, ' ', COLS);
    hblank[COLS] = 0;
...
int do_justify(void)
{
	...
	current->data = (char *)nrealloc(current->data, len + len2 + 2);
	current->data[len++] = ' ';
	current->data[len] = '\0';
	...
	do {
	    int i = 0;
	    int len2 = 0;
	    filestruct *tmpline = (filestruct *)nmalloc(sizeof(filestruct));
	    ...
	    tmpline->data = (char *)nmalloc(len2 + 1);

	    /* Skip the white space in current. */
	    memcpy(tmpline->data, current->data + i + 1, len2);
	    tmpline->data[len2] = '\0';

	    current->data = (char *)nrealloc(current->data, i + 1);
...
void help_init(void)
{
    ...
    help_text = (char *)nmalloc(allocsize);
...
#ifndef DISABLE_SPELLER
	case 's':
	    alt_speller = (char *)nmalloc(strlen(optarg) + 1);
...
*/
	...
	if (argv[optind][0] == '+') {
		...
		filename = (char *)mallocstrcpy(filename, argv[optind]);
