/* $Id: search.c,v 1.56 2001/03/18 18:55:06 astyanax Exp $ */
/**************************************************************************
 *   search.c                                                             *
 *                                                                        *
...
{
    if (last_search == NULL) {
	last_search = (char *)nmalloc(1);
	last_search[0] = 0;
    }
    if (last_replace == NULL) {
	last_replace = (char *)nmalloc(1);
	last_replace[0] = 0;
    }
}
...
int search_init(int replacing)
{
    ...
    buf = (char *)nmalloc(strlen(last_search) + 5);
    buf[0] = 0;
	...
	if (backupstring == NULL || !strcmp(backupstring, last_search))
	    backupstring = (char *)mallocstrcpy(backupstring, "");
    }
    else if (backupstring == NULL)
	backupstring = (char *)mallocstrcpy(backupstring, last_search);
	...
	free(backupstring);
	backupstring = NULL;
	backupstring = (char *)mallocstrcpy(backupstring, answer);
...
int do_search(void)
{
    ...
    if (!strcmp(answer, ""))
	answer = (char *)mallocstrcpy(answer, last_search);
    else
	last_search = (char *)mallocstrcpy(last_search, answer);
...
char *replace_line(void)
{
    ...
    /* Create buffer */
    copy = (char *)nmalloc(new_line_size);
			...
			int wholewords, int *i)
{
	...
	if (strcmp(last_replace, ""))
	    answer = (char *)mallocstrcpy(answer, last_replace);
    ...
    last_replace = (char *)mallocstrcpy(last_replace, answer);
    while (1) {
...
int do_replace(void)
{
    ...
    if (!strcmp(answer, "")) {
	answer = (char *)mallocstrcpy(answer, last_search);
	prevanswer = (char *)mallocstrcpy(prevanswer, last_search);
    } else {
	last_search = (char *)mallocstrcpy(last_search, answer);
	prevanswer = (char *)mallocstrcpy(prevanswer, answer);
    }

    if (ISSET(PICO_MODE)) {
	buf = (char *)nmalloc(strlen(last_replace) + 5);
	if (strcmp(last_replace, "")) {
