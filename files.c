/* $Id: files.c,v 1.101 2001/03/18 18:55:06 astyanax Exp $ */
/**************************************************************************
 *   files.c                                                              *
 *                                                                        *
...
filestruct *read_line(char *buf, filestruct * prev, int *line1ins)
{
    ...
    fileptr = (filestruct *)nmalloc(sizeof(filestruct));
    fileptr->data = (char *)nmalloc(strlen(buf) + 2);
...
int read_file(int fd, char *filename)
{
	...
	if (input[0] == '\n') {
	    ...
	    if (i >= bufx - 1) {
		buf = (char *)nrealloc(buf, bufx + 128);
		bufx += 128;
	    }
	    buf[i] = input[0];
	    buf[i + 1] = 0;
...
#else
	realname = (char *)mallocstrcpy(realname, answer);
#endif
...
#endif
	    if 	(tmp != NULL)
		realname = (char *)mallocstrcpy(realname, tmp);
	    else
...
int write_file(char *name, int tmp)
{
...
#else
    realname = (char *)mallocstrcpy(realname, name);
#endif
    ...
    else if (ISSET(FOLLOW_SYMLINKS) || tmp) {
	...
	buf = (char *)nmalloc(strlen(realname) + 8);
    ...
    */
    ...
    if (!tmp) {
	filename = (char *)mallocstrcpy(filename, realname);
...
int do_writeout(char *path, int exiting)
{
    ...
    answer = (char *)mallocstrcpy(answer, path);
	...
	if (i == NANO_TOFILES_KEY) {
	    ...
	    if (tmp != NULL)
		answer = (char *)mallocstrcpy(answer, tmp);
	    else
...
char *real_dir_from_tilde(char *buf)
{
    ...
    dirtmp = (char *)mallocstrcpy(dirtmp, buf);
	...
	if (buf[1] == 0 || buf[1] == '/') {
		...
		free(dirtmp);
		dirtmp = (char *)nmalloc(strlen(buf) + 2 + strlen(getenv("HOME")));
	    ...
	    find_user = (char *)mallocstrcpy(find_user, &buf[1]);
	    find_user[i - 1] = 0;
	    ...
	    if (userdata != NULL) {  /* User found */
	        ...
	        free(dirtmp);
		dirtmp = (char *)nmalloc(strlen(buf) + 2 + strlen(userdata->pw_dir));
...
char **username_tab_completion(char *buf, int *num_matches)
{
    ...
    *num_matches = 0;
	...
	if (check_wildcard_match(userdata->pw_name, &buf[1]) == TRUE) {
	    ...
	    matchline = (char *)nmalloc(strlen(userdata->pw_name) + 2);
...
char **cwd_tab_completion(char *buf, int *num_matches)
{
	...
	dirName = (char *)nmalloc(strlen(buf) + 1);
	tmp = buf + strlen(buf);
	while (*tmp != '/' && tmp != buf)
	    tmp--;
	...
	*num_matches = 0;
	...
	if (check_wildcard_match(next->d_name, tmp) == TRUE) {
	     ...
	     */
	    tmp2 = NULL;
	    tmp2 = (char *)nmalloc(strlen(next->d_name) + 1);
...
char *input_tab(char *buf, int place, int *lastWasTab, int *newplace)
{
	...
	*lastWasTab = 1;
	...
	/* Make a local copy of the string -- up to the position of the
	   cursor */
	matchBuf = (char *) (char *)nmalloc((strlen(buf) + 2) * sizeof(char));
	...
	case 1:
	    ...
	    buf = (char *)nrealloc(buf, strlen(buf) + strlen(matches[0]) + 1);
	...
	default:
		...
		if (match_matches == num_matches &&
		    ...
		    buf = (char *)nrealloc(buf, strlen(buf) + 2);
	...
	if (matches && num_matches > 0) {
	    ...
	    foo = (char *)nmalloc(longestname + 5);
...
char **browser_init(char *path, int *longest, int *numents)
{
    ...
    *longest += 10;
	...
	if (!strcmp(next->d_name, "."))
	   continue;
	filelist[i] = (char *)nmalloc(strlen(next->d_name) + strlen(path) + 2);
...
char *do_browser(char *inpath)
{
    ...
    if (path == NULL)
	path = (char *)mallocstrcpy(path, inpath);

    filelist = browser_init(path, &longest, &numents);
    foo = (char *)nmalloc(longest + 8);
	...
	case 'S':
		...
		path = (char *)mallocstrcpy(path, filelist[selected]);
...
char *do_browse_from(char *inpath)
{
    ...
    tmp = (char *)mallocstrcpy(tmp, inpath);
